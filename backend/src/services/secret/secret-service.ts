...
export const secretServiceFactory = ({
  ...
  // Expiry lives on the v2 bridge tables only.
  const getExpiringSecrets = async (dto: TGetExpiringSecretsDTO) => {
    ...
    if (project.version !== ProjectVersion.V3)
      throw new BadRequestError({ message: "Secret expiry is not supported on this project version" });
